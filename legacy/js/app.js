...
(function() {
  ...
  function renderCardsGrid(cards) {
    ...
    cards.forEach((card, index) => {
      ...
      // 获取塔罗牌图片 URL
      const imageUrl = window.TAROT_IMAGES ? window.TAROT_IMAGES.getImageUrl(card) : null;

      // 构建卡牌正面 HTML（含图片）
      const imgHTML = imageUrl
        ? `<div class="card-img-wrap">
            <div class="card-img-loading">🔮</div>
            <img
              src="${imageUrl}"
        ...
        : `<div class="card-img-wrap img-error">
  ...
  function handleCardPick(wrapper, card, index) {
    ...
    // 逆位时给 wrapper 添加类名，图片和标签文字旋转180度
    if (isReversed) {
      wrapper.classList.add('is-reversed');
    }
